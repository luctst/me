...
export function DataTable<TData>(props: Props<TData>) {
  ...
  return (
    <Table
      className={cn(isMounted ? 'animate-[fadeIn_500ms_ease_forwards]' : null)}
      style={{
        transform: 'translateY(100%)',
        animationDelay: `${4000 + 250}ms`,
      }}
    >
      <TableHeader>
            ...
            {headerGroup.headers.map((header) => {
              return (
                <TableHead
                  ...
                  className={cn(
                    header.column.columnDef.meta?.headerCustomClass,
                  )}
                >
              ...
              >
                {row.getVisibleCells().map((cell) => (
                  <TableCell
                    key={cell.id}
                    className={cn(
                      'group-hover:bg-background hover:bg-background bg-background',
                      cell.column.columnDef.meta?.cellCustomClass,
                    )}
              ...
              {row.getIsExpanded() ? (
                <TableRow className="bg-transparent" key={`${row.id}-expanded`}>
                  <TableCell
                    ...
                    className="p-0 relative whitespace-normal"
                  >
                    {innerRow(row)}
                  </TableCell>
                </TableRow>
